  ...
  for (int i = 0; i < int(in.size()); i++) {
    ...
    if (test == token) {
      if (!temp.empty()) {
        ...
        // Fix: Only push empty string if token is NOT a space.
        // For spaces, we want to treat multiple spaces as a single delimiter.
        if (token != " ") {
          out.push_back("");
        }
      }
...
inline std::string tail(const std::string &in) {
  size_t token_start = in.find_first_not_of(" \t\r");
  size_t space_start = in.find_first_of(" \t\r", token_start);
  size_t tail_start = in.find_first_not_of(" \t\r", space_start);
  size_t tail_end = in.find_last_not_of(" \t\r");
...
inline const T &getElement(const std::vector<T> &elements, std::string &index) {
  if (index.empty())
    return elements[0]; // Error: Empty index
  int idx = 0;
  try {
    idx = std::stoi(index);
  } catch (...) {
  ...
  bool LoadMaterials(std::string path) {
    ...
    while (std::getline(file, curline)) {
      ...
      if (algorithm::firstToken(curline) == "illum") {
        std::string val = algorithm::tail(curline);
        if (!val.empty()) {
          try {
            tempMaterial.illum = std::stoi(val);
          } catch (...) {
          }
        }
      }
      ...
      if (algorithm::firstToken(curline) == "Pr") {
        tempMaterial.Pr = std::stof(algorithm::tail(curline));
      }
      // Metallic
      if (algorithm::firstToken(curline) == "Pm") {
        tempMaterial.Pm = std::stof(algorithm::tail(curline));
      }
      // Transparency
      if (algorithm::firstToken(curline) == "Tr") {
        tempMaterial.Tr = std::stof(algorithm::tail(curline));
        // Update Dissolve if Tr is present (d = 1 - Tr)
        // But usually d takes precedence if both exist.
        // Let's store it and handle priority in mesh.cpp
      }
    }
