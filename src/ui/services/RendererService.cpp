...
using ProgressCallback = std::function<void(const QImage &,
                                            const render::TileProgressStats &,
                                            int stageIndex,
                                            int totalStages,
                                            const StageSettings &stage)>;
using StageChangedCallback = std::function<void(const QString &, int stageIndex, int totalStages, const StageSettings &stage)>;
...
{
    const QString candidate = input.isEmpty() ? QStringLiteral("objects.xml") : input;
    QFileInfo info(candidate);
...
std::vector<StageSettings> buildStages(const RenderJobConfig &config, world &sceneWorld)
{
    int baseWidth = config.width > 0 ? config.width : sceneWorld.GetImageWidth();
    int baseHeight = config.height > 0 ? config.height : sceneWorld.GetImageHeight();
    const double aspectRatio = sceneWorld.GetAspectRatio() > 0.0
        ? sceneWorld.GetAspectRatio()
        : (baseHeight > 0 ? static_cast<double>(baseWidth) / static_cast<double>(baseHeight) : 16.0 / 9.0);
    if (config.width > 0 && config.height <= 0) {
        baseHeight = static_cast<int>(std::round(static_cast<double>(baseWidth) / aspectRatio));
    } else if (config.height > 0 && config.width <= 0) {
        baseWidth = static_cast<int>(std::round(static_cast<double>(baseHeight) * aspectRatio));
    }
    const int baseSamples = config.samples > 0 ? config.samples : sceneWorld.GetSamplesPerPixel();

    std::vector<StageSettings> stages;

    const bool enablePreview = baseWidth >= 512 || baseHeight >= 512 || baseSamples >= 16;
    if (enablePreview) {
        const double aspect = aspectRatio;
        int previewWidth = std::max(256, baseWidth / 2);
        int previewHeight = static_cast<int>(std::round(static_cast<double>(previewWidth) / aspect));
        previewHeight = std::max(144, previewHeight);
        const int previewSamples = std::max(1, baseSamples / 4);
        stages.push_back({previewWidth, previewHeight, previewSamples, QObject::tr("Preview"), false});
    }

    stages.push_back({baseWidth, baseHeight, baseSamples, QObject::tr("Final"), true});
    return stages;
}

QImage bitmapToImage(const std::vector<color> &bitmap,
                     int width,
                     int height,
                     int samplesPerPixel)
{
                          ...
                          StageChangedCallback stageChanged)
{
    ...
    if (!worldPtr) {
        const QString message = QObject::tr("Unable to load scene: %1").arg(resolvedPath);
    ...
    const unsigned int fallback = idealQtCount > 0 ? static_cast<unsigned int>(idealQtCount) : 1u;
    const unsigned int threads = hwThreads ? hwThreads : fallback;

    // Set acceleration method based on config
    worldPtr->pconfig->acceleration = config.useBVH ? AccelerationMethod::BVH : AccelerationMethod::LINEAR;
    ...
    for (int stageIndex = 0; stageIndex < totalStages; ++stageIndex) {
        ...
        worldPtr->pconfig->ASPECT_RATIO = stage.height > 0
            ? static_cast<double>(stage.width) / static_cast<double>(stage.height)
        ...
        if (progressCallback) {
                 tileCallback = [progressCallback,
                            ...
                            stageIndex,
                            totalStages,
                            stage](const std::vector<color> &bitmap,
                                   int /*width*/,
                                   int /*height*/,
                ...
                const QImage frame = bitmapToImage(bitmap, stageWidth, stageHeight, stageSamples);
                if (frame.isNull()) {
                    return;
                }
        ...
        render::RenderSceneToBitmap(*worldPtr, bitmap, threads, 64, false, tileCallback, cancelFlag);
        ...
        if (stage.finalStage) {
            result.frame = bitmapToImage(bitmap, stage.width, stage.height, stage.samples);
...
RendererService::RendererService(QObject *parent)
    : QObject(parent)
{
    qRegisterMetaType<RenderTelemetry>("Raytracer::ui::RendererService::RenderTelemetry");
...
void RendererService::startRender(const RenderRequest &request)
{
    ...
    RenderJobConfig config {request.scenePath, request.width, request.height, request.samplesPerPixel, request.useBVH};
    ...
    auto progressCallback = [this, renderStart, stageStart, request](const QImage &frame,
                                                                       const render::TileProgressStats &stats,
                                                                       int stageIndex,
                                                                       int totalStages,
                                                                       const StageSettings &stage) {
        const double stageCompletion = stats.totalTiles > 0
            ? static_cast<double>(stats.tilesDone) / static_cast<double>(stats.totalTiles)
            : 0.0;
        const double overallCompletion = std::min(1.0,
            (static_cast<double>(stageIndex) + stageCompletion) /
        ...
        telemetry.tilesPerSecond = stats.avgTileMs > 0.0 ? 1000.0 / stats.avgTileMs : 0.0;
        ...
        QMetaObject::invokeMethod(this, [this, frame = std::move(processedFrame), stats, telemetry, stageCompletion]() {
            emit frameReady(frame);
            emit progressUpdated(telemetry.overallProgress, stats.estRemainingMs / 1000.0);
    ...
    auto stageChanged = [this, stageStart](const QString &label, int /*stageIndex*/, int /*totalStages*/, const StageSettings &/*stage*/) {
        stageStart->restart();
