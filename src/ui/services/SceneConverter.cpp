...
SceneConverter::convertToWorld(const SceneDocument *document) {
  clearError();
  ...
  for (const auto &lightSettings : document->pointLights()) {
    if (lightSettings.enabled) {
      point3 pos(lightSettings.position.x, lightSettings.position.y,
                 lightSettings.position.z);
      color col(lightSettings.color.x, lightSettings.color.y,
                lightSettings.color.z);
      auto light =
          std::make_shared<PointLight>(pos, col, lightSettings.intensity);
      pworld->pointLights.push_back(light);
    }
  }
...
SceneConverter::convertConfig(const SceneDocument *document) {
...
SceneConverter::convertCamera(const SceneDocument *document,
                              float aspectRatio) {
...
std::shared_ptr<sun> SceneConverter::convertSun(const SceneDocument *document) {
  ...
  vec3 direction(sunSettings.direction.x, sunSettings.direction.y,
...
SceneConverter::convertMaterial(const MaterialDefinition *matDef) {
  if (!matDef)
    return nullptr;
  ...
  case MaterialType::Dielectric:
    ...
    return std::make_shared<dielectric>(matDef->refractiveIndex());
...
std::shared_ptr<material>
SceneConverter::getMaterial(const QUuid &materialId,
...
SceneConverter::convertNode(const SceneNode *node,
                            const SceneDocument *document) {
...
std::shared_ptr<hittable>
SceneConverter::convertSphere(const SceneNode *node,
                              const SceneDocument *document) {
  if (!node)
    return nullptr;
...
std::shared_ptr<hittable>
SceneConverter::convertTriangle(const SceneNode *node,
                                const SceneDocument *document) {
  if (!node)
    return nullptr;
...
std::shared_ptr<hittable>
SceneConverter::convertMesh(const SceneNode *node,
                            const SceneDocument *document) {
  if (!node)
    return nullptr;
  ...
  auto meshObj = std::make_shared<mesh>(meshFilePath.toStdString(), position,
                                        scaleVec, rotation, mat);
