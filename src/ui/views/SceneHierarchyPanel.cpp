...
void SceneTreeModel::setSceneDocument(SceneDocument *document)
{
    ...
    if (m_document) {
        connect(m_document, &SceneDocument::documentChanged, this, &SceneTreeModel::onSceneChanged);
        connect(m_document, &SceneDocument::nodeAdded, this, [this](SceneNode* node) {
            onNodeAdded(node ? node->uuid() : QUuid());
        });
        connect(m_document, &SceneDocument::nodeRemoved, this, &SceneTreeModel::onNodeRemoved);
        connect(m_document, &SceneDocument::nodeChanged, this, [this](SceneNode* node) {
...
{
    if (!m_document || column != 0) return QModelIndex();
...
{
    if (!index.isValid() || !m_document) return QModelIndex();
    ...
    if (!grandparent) grandparent = m_document->rootNode();
...
QMimeData* SceneTreeModel::mimeData(const QModelIndexList &indexes) const
{
    ...
    mimeData->setData(QStringLiteral("application/x-raytracer-scenenode"), encodedData);
    return mimeData;
}
                                  ...
                                  int row, int column, const QModelIndex &parent)
{
    ...
    if (!data->hasFormat(QStringLiteral("application/x-raytracer-scenenode"))) return false;
    if (!m_document) return false;
    
    QByteArray encodedData = data->data(QStringLiteral("application/x-raytracer-scenenode"));
    QDataStream stream(&encodedData, QIODevice::ReadOnly);
    
    SceneNode *newParent = parent.isValid() 
        ? static_cast<SceneNode*>(parent.internalPointer()) 
        : m_document->rootNode();
...
{
    m_addSphereAction = new QAction(QIcon::fromTheme("draw-circle"), tr("Add Sphere"), this);
    connect(m_addSphereAction, &QAction::triggered, this, &SceneHierarchyPanel::addSphere);
    
    m_addTriangleAction = new QAction(QIcon::fromTheme("draw-triangle"), tr("Add Triangle"), this);
    connect(m_addTriangleAction, &QAction::triggered, this, &SceneHierarchyPanel::addTriangle);
    
    m_addMeshAction = new QAction(QIcon::fromTheme("draw-polygon"), tr("Add Mesh"), this);
    connect(m_addMeshAction, &QAction::triggered, this, &SceneHierarchyPanel::addMesh);
    
    m_addGroupAction = new QAction(QIcon::fromTheme("folder"), tr("Add Group"), this);
    connect(m_addGroupAction, &QAction::triggered, this, &SceneHierarchyPanel::addGroup);
    
    m_deleteAction = new QAction(QIcon::fromTheme("edit-delete"), tr("Delete"), this);
    m_deleteAction->setShortcut(QKeySequence::Delete);
    connect(m_deleteAction, &QAction::triggered, this, &SceneHierarchyPanel::deleteSelected);
    
    m_duplicateAction = new QAction(QIcon::fromTheme("edit-copy"), tr("Duplicate"), this);
    m_duplicateAction->setShortcut(QKeySequence(Qt::CTRL | Qt::Key_D));
    connect(m_duplicateAction, &QAction::triggered, this, &SceneHierarchyPanel::duplicateSelected);
    
    m_renameAction = new QAction(tr("Rename"), this);
    m_renameAction->setShortcut(Qt::Key_F2);
    connect(m_renameAction, &QAction::triggered, this, &SceneHierarchyPanel::renameSelected);
}
...
void SceneHierarchyPanel::setSelectionManager(SelectionManager *manager)
{
    ...
    if (m_selectionManager) {
        connect(m_selectionManager, &SelectionManager::selectionChanged, 
                this, &SceneHierarchyPanel::syncSelectionFromManager);
    }
}
...
{
    if (m_updatingSelection || !m_selectionManager) return;
...
void SceneHierarchyPanel::syncSelectionToManager()
{
    ...
    for (const QModelIndex &idx : m_treeView->selectionModel()->selectedIndexes()) {
...
void SceneHierarchyPanel::onSelectionChanged(const QItemSelection &selected, const QItemSelection &deselected)
{
    Q_UNUSED(selected);
    Q_UNUSED(deselected);
...
void SceneHierarchyPanel::onCustomContextMenu(const QPoint &pos)
{
    ...
    bool hasSelection = !m_treeView->selectionModel()->selectedIndexes().isEmpty();
...
{
    auto *doc = sceneDocument();
    if (!doc || !m_selectionManager) return;
    
    auto selected = m_selectionManager->selectedNodes();
    if (selected.empty()) return;
    
    if (m_commandHistory && selected.size() > 1) {
        m_commandHistory->beginMacro(QStringLiteral("Delete %1 objects").arg(selected.size()));
    }
